...
$\boldsymbol{U}^T y$ 表示 $\hat{y}$ 在以 $\boldsymbol{U}$ 的列向量为 basis 的空间内的坐标，所以 $\hat{y} = \boldsymbol{U}\boldsymbol{U}^T y$。

这个可以这么理解的，考虑正常的 3d space 中的坐标，比如 $v = (3, 4, 5)^T$，它实际上是等于

$$3 \begin{pmatrix} 1 \\\\ 0\\\\ 0 \end{pmatrix} + 4 \begin{pmatrix} 0 \\\\ 1 \\\\ 0 \end{pmatrix} + 5 \begin{pmatrix} 0 \\\\ 0 \\\\ 1 \end{pmatrix} = \boldsymbol{I} v$$

其中 $I$ 为 basis，同理 $y$ 在 $\boldsymbol{U}$ 中的坐标 $\boldsymbol{U}^T y$，其 basis 为 $\boldsymbol{U}$，所以 $\hat{y} = \boldsymbol{U}\boldsymbol{U}^T y$。

#### (3.49)

首先注意这个公式里的 $Var(\boldsymbol{z}\_1)$ 不是个 covariance matrix，这里的向量 $\boldsymbol{z}\_1$ 表示的是一个 random variable 的 n 个取值，而不是 n 个 random variable 分别的取值，搞清楚这个非常重要。情况是这样，$\boldsymbol{X}$ 是原始空间中的样本，原始空间中每个样本有 p 个 feature，也就是 p 个 random variable，而 $\boldsymbol{X}$ 的每一列分别表示这 p 个 feature 的 n 个取值，而 $\boldsymbol{X}v\_1$ 将 p 个 feature project 到一个新的空间中，变成一个 feature，$\boldsymbol{z}\_1$ 就是 n 个样本对这个 feature 的取值，所以说它是一个 random variable 的 n 个取值。

令 $\boldsymbol{X}\_i$ 表示 $\boldsymbol{X}$ 的第 i 列，由于 $\boldsymbol{X}$ 是 centered，所以 $\sum\_{j=1}^{n} \boldsymbol{X}\_{ij} = 0$，由此

$$ Mean(\boldsymbol{X}v\_1) = \frac{\sum\_{i = 1}^{p} \sum\_{j = 1}^{n} \boldsymbol{X}\_{ij}v\_{1i}}{N} = 0$$

所以 $\boldsymbol{X}v\_1$ 的 sample variance 就是

$$\frac{(\boldsymbol{X}v\_1)^T\boldsymbol{X}v\_1}{N} = \frac{v\_1^T\boldsymbol{X}^T \boldsymbol{X}v\_1}{N} = \frac{v\_1^T {d\_1}^2 v\_1}{N} = \frac{d\_1^2}{N}$$

#### (3.49) 下面的公式

$\boldsymbol{z}\_1 = \boldsymbol{X} v\_1 = \boldsymbol{u}\_1 d\_1$ 的推导很简单，只要把 $\boldsymbol{X}$ 对应的 SVD 分解代入即可。

### 注意点

* ( **page 66** ) 注意区分 principal component 和 principal component direction，principal component direction 是 $\boldsymbol{X}^T \boldsymbol{X}$ 的 eigenvector，假设为 $v$，$\boldsymbol{X}v$ 才是 principal component，如 Figure 3.9 所示。

* ( **page 72** ) figure 3.12 可以看到 q 从 0.1 到 4 的过程中，contour 的图形变化从窄到越来越方。

* ( **page 72** ) 第 3 段里提到 "for $q \leq 1$, the prior is not uniform in direction, but concentrates more mass in the coordinate directions"，这句话是这么理解的，这里的 mass 就是 probability mass，根据 L1 norm 的 contour 想象一下 L1 norm 对应的 laplace prior (即 $c \exp(-\lambda \sum\_{j = 1}^{p} |\beta\_j|)$，$c$ 是个 normalization factor，注意这里有个负号，因为 gaussian likelihood 也有个负号，两个的 log 相加做个 argmax 就变成了这里的 argmin) 的形状，沿着某一方向的 probability mass 就是沿着那个方向对 probability density 做积分，而这实际上就是沿着那个方向对 probability density 做截面得到的截面面积，显然沿着坐标轴截得的的平面 (是个三角形) 的面积是最大的，也就是说 laplace prior 更倾向于坐标轴上的点。

  这也是 L1 regularization 倾向于将某些 feature weight 置为 0 的一种解释，另一种是从优化的角度解释，如 figure 3.11 所示。
