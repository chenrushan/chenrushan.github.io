...
* **(page 102)** 公式 (4.2) 下面有这么一句 "The decision boundary is the set of points for which the log-odds are zero"，其实这个可以直接从 $Pr(G=1|X=x) = Pr(G=2|X=x)$ 推出来，这个等式直接就推出 $\\{x|\beta\_0 + \beta^T x = 0\\}$
